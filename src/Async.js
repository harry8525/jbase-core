(function ()
{
    ...
    defineNamespace(
        "Shared.Async",
        {
            ...
            asyncRepeat: function (count, callback)
            {
                /// <summary>Asyncronously calls a callback for {count} times, batching them to avoid freezing the ui thread.</summary>
                /// <param name="count" type="Number">Number of iterations.</param>
                /// <param name="callback" type="Function">Function to execute, passes in current index to arguments[0].</param>
                /// <returns type="Shared.Promise">Promise that can be canceled.</returns>

                var context = Qos.getContext();

                callback = Qos.instrument(context, callback);
                ...
                return new Shared.Promise(
                    function onExecute(complete, error)
                    {
                        function executeIteration()
                        {
                            ...
                            if (retVal === false || i == count)
                            {
                                // Ensure we return false by setting i to count if retVal === false.
                                i = count;
                                complete();
                            }
                        ...
                        if (count > 0)
                        {
                            _enqueue(enqueueId, _this, executeIteration, false);
                        }
                        else
                        {
                            complete();
                    ...
                    function onCancel()
                    {
                        _cancelEnqueue(enqueueId);
                    });
            },

            throttle: function (id, waitTime, maxWaitTime, callback, callImmediately)
            {
            ...
            _callThrottledCallback: function (id)
            {
                ...
                if (_this._throttledFunctions)
                {
                    ...
                    if ((timeSinceLastExecute >= throttleRecord.waitTime) || (throttleRecord.maxWaitTime && timeSinceStart >= throttleRecord.maxWaitTime))
                    {
                        ...
                        {
                            throttleRecord.callback.apply(_this);
                        }
                    }
                    else
                    {
    ...
    function _enqueue(id, caller, callback, isForegroundTask)
    {
        /// <summary>Enqueues a task into the right queue.</summary>
        ...
        (isForegroundTask ? _foregroundTasks : _backgroundTasks).push(task);
        _allTasks[id] = task;

        _ensureProcessing();
    }

    function _cancelEnqueue(id)
    {
        /// <summary>Deletes the task entry so that it isn't executed.</summary>

        delete _allTasks[id];
    }

    function _ensureProcessing()
    {
        /// <summary>Ensures that if we have tasks enqueued and no timeout being executed, execute the method.</summary>

        if (!_queueTimeoutId && /* @static_cast(Boolean) */(_foregroundTasks.length || _backgroundTasks.length))
        {
            _queueTimeoutId = setTimeout(_processQueue, c_timeBetweenIterations);
        }
    }

    function _processQueue()
    {
        /// <summary>Processes items in the queue while we have time left.</summary>

        var /* @dynamic */task;
        var duration = 0;
        var itemsProcessed = 0;
        _queueTimeoutId = 0;
        _executionStartTime = new Date().getTime();

        // Process foreground tasks. If they return true, shift them back to the front. Foreground tasks are able to block other tasks until they are done.
        while (/* @static_cast(Boolean) */_foregroundTasks.length && itemsProcessed < c_maxItemsPerIteration && duration < c_maxDuration)
        {
            task = _foregroundTasks[0];
            if (_allTasks[task.id])
            {
                if (!task.callback.apply(task.caller))
                {
                    task = _foregroundTasks.shift();
                    delete _allTasks[task.id];
                }
                itemsProcessed++;
            }
            else
            {
                _foregroundTasks.shift();
            }

            duration = (new Date().getTime() - _executionStartTime);
        }

        // Process background tasks. If they return true, push them to the back. Background tasks get processed in round-robbin.
        while (/* @static_cast(Boolean) */_backgroundTasks.length && itemsProcessed < c_maxItemsPerIteration && duration < c_maxDuration)
        {
            task = _backgroundTasks.shift();
